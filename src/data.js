...
const PROJECTS = [
  ...
  {id:'syncoxide',name:'SyncOxiders',kind:'oss',cats:['fs'],live:'https://syncoxide.rs',
   ...
   body:['SyncOxiders syncs files and email across cloud providers and adds inter-cloud encryption, sharing and backup — an end-to-end encrypted layer over the storage you already use.',
         'Written in Rust with a focus on safety and performance. Its peer-to-peer browser transfer ships as Oxfer, a standalone web app.']},
  {id:'oxfer',name:'Oxfer',kind:'app',cats:['fs','realtime'],shot:'oxfer',fit:'top',
   live:'https://oxfer.pages.dev/',repo:'https://github.com/radumarias/syncoxiders/tree/main/p2p-transfer',issues:'https://github.com/radumarias/syncoxiders/issues',tags:['rust','p2p'],tech:['Rust','WebAssembly','egui','iroh','WebRTC','BLAKE3'],
   blurb:'Browser-to-browser file transfer — your files, their device, no cloud in between.',
   body:['Pick a file, share one private link, and the receiver downloads it straight from your browser — no accounts, no upload, no server-side storage. Files are hashed locally with BLAKE3 and streamed in bounded chunks, so large transfers go straight to disk while memory stays flat.',
         'Written in Rust with egui and compiled to WebAssembly (it also runs as a native desktop app). An authenticated iroh connection over QUIC carries the signalling, then the browsers switch to a direct, DTLS-encrypted WebRTC data channel — falling back to the iroh relay, which never sees plaintext, when no direct path exists. Dropped connections reconnect automatically, and every file is verified before it counts as saved.']},
...
const GROUPS = [
  ...
  {title:'Filesystems, sync & crypto',ids:['rencfs','syncoxide','oxfer','rfs','rencfs-desktop','rencfs-daemon','ciphershell','gdrive-rs','in-mem-fs','fuse3-template','rencrypt','zeroize','crypto-bench']},
